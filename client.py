...
def print_message_box(header, lines):
    ...
    rows = [border, header_row, border] + content_rows + [border]
        ...
        while True:
            ...
            if 'public_keys' in data:
                ...
                if data['step'] == 1:
                    ...
                    bin_key = des.string2bin(session_des_key)[0]
                    session_round_key = des.generate_round_keys(bin_key)[0]

                    selected_public_keys = public_keys[bring_back_to]

                    encrypted_step_4 = rsa.encoder(
                    ...
                    for i in range(0, len(encrypted_bin_message), 64):
                        ...
                        decrypted_chunk = des.decrypt(chunk, session_round_key)
                        decrypted_bin_message += decrypted_chunk

                    # Trim the message to its original length
                    decrypted_message = des.bin2string(decrypted_bin_message)

                    # Print the message in a box :)
                    header = f"Received from {source}:"
                    lines = [
                        f"Raw           : {des.bin2string(encrypted_bin_message)}",
        ...
        while True:
            ...
            if state == 'listen':
                ...
                if message.lower() == 'b':
                    ...
                    bin_message = des.string2bin(message)
                    encrypted_bin_message = ''
                    for chunk in bin_message:
                        encrypted_bin_message += des.encrypt(chunk,
                                                         session_round_key)
